...
class kbTokenizer:
  ...
  def tokenizeSentence(self, sString):
    ...
    if self.bLowerCase:
      aTokens = \
          self.reWhitespace.split(self.removeNonTokenChars(sString.lower()))
    else:
      aTokens = self.reWhitespace.split(self.removeNonTokenChars(sString))
...
if __name__ == "__main__":
  import argparse
  oArgsParser = argparse.ArgumentParser(description='Tokenize a KB text file.')
